import { cross } from "../../data/repeatedPrayers.js";
...
function gregorianToCoptic(gregorianDate) {
    ...
    let copticYear;
    let diffInDays = Math.floor((gregorianDate - copticNewYearDate) / (1000 * 60 * 60 * 24));
    ...
    if (diffInDays >= 0) {
        ...
        const prevCopticNewYearDate = new Date(gregorianYear - 1, 8, isGregorianLeapYear(gregorianYear) ? 12 : 11);
        diffInDays = Math.floor((gregorianDate - prevCopticNewYearDate) / (1000 * 60 * 60 * 24));
...
function copticToGregorian(copticDate) {
    ...
    if (copticMonth === 12) { // Nasie
        if (copticDay > (nextYearLeap ? 6 : 5)) {
            throw new Error(`Invalid day in Nasie. Max is ${nextYearLeap ? 6 : 5} days.`);
        }
    ...
    const gregorianDate = new Date(copticNewYearDate.getTime() + (daysSinceNewYear * 24 * 60 * 60 * 1000));
...
const fixedFeastsCopticDates = [    
    ...
    { date: "Kiahk 29", season: "Feast of the Nativity", priority: 1 , visible: true},
    { date: "Kiahk 30", season: "2nd Day of Nativity", priority: 2 , visible: true},
    { start: "Kiahk 30", end: "Tobe 5", season: "Nativity Season", priority: 2 , visible: false},
    { date: "Tobe 6", season: "Feast of the Circumcision", priority: 2 , visible: true},
    { date: "Tobe 11", season: "Feast of the Theophany", priority: 1 , visible: true},
    { date: "Tobe 12", season: "2nd Day of Theophany", priority: 2 , visible: true},
    { date: "Tobe 13", season: "Feast of the Wedding at Cana of Galilee", priority: 2 , visible: true},
    { start: "Meshir 8", season: "Presentation of the Lord Christ in the Temple", priority: 2, visible: true },
    { date: "Paremhotep 10", season: "Feast of the Cross", priority: 2, visible: true },  
    { date: "Paremhotep 29", season: "Feast of the Annunciation", priority: 2, visible: true }, 
    { date: "Pashons 24", season: "Entrance of the Lord Christ into the Land of Egypt", priority: 2, visible: true },
    { date: "Epip 5", season: "Feast of the Apostles (Peter & Paul)", priority: 2, visible: true },
    { date: "Mesore 13", season: "Feast of the Transfiguration", priority: 2,  visible: true },
    { start: "Mesore 1", end: "Mesore 15", season: "Fast of St. Mary", priority: 3, visible: true },
    { date: "Mesore 16", season: "Assumption of the Body of St. Mary", priority: 2, visible: true },
];
function getCopticYear(copticMonthName, copticDay, gregorianYear) {
    ...
    if (copticMonthName === copticMonths[0] 
        || copticMonthName === copticMonths[1] 
        || (copticMonthName === copticMonths[2] && lastCopticDateOfYear.copticDay <= copticDay)
    ) {
...
function getFixedFeastDates(gregorianYear) {
    ...
    function calculateParamon(gregorianDate, feastName, nextYear) {
        function getUTCDateDay(gregorianDate) {
            ...
            const utcDate = new Date(Date.UTC(gregorianDate.getUTCFullYear(), gregorianDate.getUTCMonth(), gregorianDate.getUTCDate()));
    ...
    fixedFeastsCopticDates.forEach(feast => {
        if (feast.date) {
            ...
            const copticDate = {
                copticYear: getCopticYear(copticMonthName, parseInt(copticDay), feast.nextYear ? gregorianYear + 1 : gregorianYear),
                copticMonthName,
            ...
            if (feast.season === "Feast of the Nativity" || feast.season === "Feast of the Theophany") {
                // Check if Nativity needs adjustment based on Coptic leap year
                if (feast.season === "Feast of the Nativity") {
                    
                    const isLeap = isGregorianLeapYear(feast.nextYear ? gregorianYear + 1 : gregorianYear);
                ...
                const paramon = calculateParamon(gregorianDate, feast.season, feast.nextYear);
            ...
            const adjustedGregorianYear = feast.nextYear ? gregorianYear + 1 : gregorianYear;


            // Get the start and end Gregorian dates
            const copticStartDate = {
                copticYear: getCopticYear(startMonthName, parseInt(startDay), adjustedGregorianYear),
                copticMonthName: startMonthName,
                copticDay: parseInt(startDay),
            };
            const copticEndDate = {
                copticYear: getCopticYear(endMonthName, parseInt(endDay), adjustedGregorianYear),
    ...
    const kiahk = fixedFeastsGregorianDates.find(feast => feast.season === "Kiahk" && feast.end && !feast.nextYear);
    const nativityParamon = fixedFeastsGregorianDates.find(feast => feast.season === "Feast of the Nativity Paramoun" && feast.start && !feast.nextYear);
    ...
    const kiahkNextYear = fixedFeastsGregorianDates.find(feast => feast.season === "Kiahk" && feast.end && feast.nextYear);
    const nativityParamonNextYear = fixedFeastsGregorianDates.find(feast => feast.season === "Feast of the Nativity Paramoun" && feast.start && feast.nextYear);
    if (kiahkNextYear && nativityParamonNextYear) {
        kiahkNextYear.end = new Date(nativityParamonNextYear.start.getTime() - 86400000); // Subtract one day
        if (((kiahkNextYear.end - kiahkNextYear.start) / 86400000) < 24) {
            kiahkNextYear.start = new Date(kiahkNextYear.end.getTime() - 25 * 86400000); // Subtract 21 days
        }
    }
...
function getSaintFeasts(gregorianYear) {
    ...
    copticSaintFeasts.forEach(feast => {
        if (feast.date) {
            ...
            const [copticMonthName, copticDay] = feast.date.split(" ");
            const copticDate = {
                copticYear: getCopticYear(copticMonthName, parseInt(copticDay), gregorianYear),
                copticMonthName,
                copticDay: parseInt(copticDay),
            };
...
const apostlesFastEnd = copticToGregorian({"copticDay": 4, "copticMonthName": "Epip", "copticYear": gregorianYear-284});
...
const dynamicFeasts = [
        { start: jonahFastStart, end: jonahFastEnd, season: "Jonah's Fast" , priority: 2 , visible: true},
        { date: jonahFeast, season: "Jonah's Passover" , priority: 2, visible: true},
        { date: saturdayBeforeLent, season: "Saturday before the Great Fast" , priority: 2, visible: true},
        { date: sundayBeforeLent, season: "Sunday before the Great Fast" , priority: 2, visible: true},
        { start: lentStart, end: lentEnd, season: "Great Fast" , priority: 2 , visible: true},
        { start: lentStart, end: lentEnd, season: "First Monday of the Great Fast" , priority: 2, visible: false},
        { date: firstSundayOfLent, season: "First Sunday of the Great Fast" , priority: 1, visible: false},
        { date: secondSundayOfLent, season: "Second Sunday of the Great Fast" , priority: 1, visible: false},
        { date: thirdSundayOfLent, season: "Third Sunday of the Great Fast" , priority: 1, visible: false},
        { date: fourthSundayOfLent, season: "Fourth Sunday of the Great Fast" , priority: 1, visible: false},
        { date: fifthSundayOfLent, season: "Fifth Sunday of the Great Fast" , priority: 1, visible: false},
        { date: sixthSundayOfLent, season: "Sixth Sunday of the Great Fast" , priority: 1, visible: false},
        { date: lentEnd, season: "Last Friday of the Great Fast" , priority: 1, visible: true},
        { date: lazerusSaturday, season: "Lazarus Saturday" , priority: 1, visible: true},
        { date: palmSunday, season: "Hosanna Sunday" , priority: 1, visible: true},
        { start: holyWeekStart, end: brightSaturday, season: "Holy Week (Pascha)" , priority: 1, visible: true},
        { date: covenantThursday, season: "Covenant Thursday" , priority: 1, visible: true},
        { date: goodFriday, season: "Good Friday" , priority: 1, visible: true},
        { date: brightSaturday, season: "Joyous Saturday" , priority: 1, visible: true},
        { date: easterDate, season: "Feast of the Resurrection" , priority: 1, visible: true},
        { start: easterMonday, end: resurrectionEnd, season: "Holy 50 Days - Before Ascension" , priority: 1, visible: false},
        { date: easterMonday, season: "2nd Day of the Resurrection" , priority: 1, visible: true},
        { date: thomasSunday, season: "Thomas Sunday" , priority: 1, visible: true},
        { date: secondSundayOfPentacost, season: "Second Sunday of Pentecost" , priority: 1, visible: false},
        { date: thirdSundayOfPentacost, season: "Third Sunday of Pentecost" , priority: 1, visible: false},
        { date: fourthSundayOfPentacost, season: "Fourth Sunday of Pentecost" , priority: 1, visible: false},
        { date: fifthSundayOfPentacost, season: "Fifth Sunday of Pentecost" , priority: 1, visible: false},
        { date: ascension, season: "Feast of the Ascension" , priority: 1, visible: true},
        { date: sixthSundayOfPentacost, season: "Sixth Sunday of Pentecost" , priority: 1, visible: false},
        { start: ascensionSeasonStart, end: ascensionSeasonEnd, season: "Holy 50 Days - After Ascension" , priority: 1, visible: false},
        { date: pentecost, season: "Feast of Pentecost" , priority: 1, visible: true},
        { start: apostlesFastStart, end: apostlesFastEnd, season: "Apostles' Fast" , priority: 2, visible: true},
...
function findCopticSeason(gregorianDateAdjusted, copticSeasons) {
    ...
    for (let season of copticSeasons) {
        if (season.start && season.end) {
            ...
            if (compareDatesInUTC(gregorianDateAdjusted, startDate) >= 0 && compareDatesInUTC(gregorianDateAdjusted, endDate) <= 0) {
    ...
    let highestPriority = Math.min(...matchingSeasons.map(season => season.priority), Infinity);

    // Filter the matching seasons to include only those with the lowest priority
    let highestPrioritySeasons = matchingSeasons.filter(season => season.priority === highestPriority);
...
function getAktonkAki(today, copticSeasons) {
    ...
    const avmask = {
        ...
        end: getSeasonDate("Feast of the Circumcision", copticSeasons, false, "date")
    };

    const akoms = {
        start: getSeasonDate("Feast of the Theophany", copticSeasons, false, "date"),
    ...
    const avashk = {
        ...
        cross3start: getSeasonDate("Feast of the Cross", copticSeasons, false, "start"),
        cross3end: getSeasonDate("Feast of the Cross", copticSeasons, false, "end"),
        paschaStart: getSeasonDate("Feast of the Cross", copticSeasons, false, "start"),    
    ...
    const aktonk = {
        start: getSeasonDate("Feast of the Resurrection", copticSeasons, false,"date"),
        end: getSeasonDate("Feast of Pentecost", copticSeasons, false, "date"),
        sundayEnd: getSeasonDate("Fast of the Nativity - Advent", copticSeasons, true, "start"),
    };
    ...
    // Other periods (avmask, akoms, avashk)
        ...
        aktonkAki = "Akoms";
    } else if ((today >= avashk.cross3start && today <= avashk.cross3end) || (today >= avashk.paschaStart && today <= avashk.paschaEnd) || (today === avashk.cross1)) {
