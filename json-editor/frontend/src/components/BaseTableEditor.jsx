...
const BaseTableEditor = ({
  ...
  const removeRowField = useCallback((tableIndex, tbodyIndex, rowIndex, field) => {
    updateTable(tableIndex, (tbl) => {
      const next = { ...tbl };
      const tbodies = Array.isArray(tbl.tbodies) ? tbl.tbodies.map((tb) => ({ ...tb })) : [];
      const tbody = tbodies[tbodyIndex] || { rows: [] };
    ...
    <div className="hw-tables">
      ...
      <div className="hw-table-list">
        {tables.map((tbl, idx) => {
          ...
          const optionalSelected = baseOptionalSelect[optionalKey] || "";
          ...
          const renderOptionalInput = (field) => {
            ...
            const labelText = tableFieldLabels?.[field] || field;
            if (isBool) {
              return (
            ...
            const selectOptions = tableFieldOptions?.[field];
            if (Array.isArray(selectOptions)) {
              return (
                <label key={field} className="hw-field-with-remove hw-field-inline">
                  <span>{labelText}</span>
                  <select value={value == null ? "" : String(value)} onChange={(e) => setTableField(idx, field, e.target.value)}>
                    <option value="" disabled>
                      Choose a value
                    </option>
          ...
          return (
            ...
            >
              ...
              <div className="hw-table-fields">
                {!isPlaceholder && (
                  <>
                    <label className="hw-field-inline">
                    ...
                    {optionalChoices.length > 0 && (
                      <div className="hw-optional hw-optional-inline">
                        <select
                        ...
                        <button
                          type="button"
                          className="secondary"
                          onClick={() => addOptionalField(idx, optionalSelected)}
                        >
                          Add variable
                        </button>
                      </div>
