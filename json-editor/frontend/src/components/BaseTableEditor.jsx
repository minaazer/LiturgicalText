  ...
  const removeRepeatedVariable = useCallback((tableIndex, key) => {
    updateTable(tableIndex, (tbl) => {
      ...
      if (next.repeated_prayer_variables) {
        ...
        if (key === "passToTable.dayOfTheWeek" && vars.passToTable) {
          const pass = { ...vars.passToTable };
          delete pass.dayOfTheWeek;
          vars.passToTable = pass;
        } else {
          delete vars[key];
        }
    ...
    <div className="hw-tables">
      ...
      <div className="hw-table-list">
        {tables.map((tbl, idx) => {
          ...
          const isLinkOnly = "link" in (tbl || {}) && Object.keys(tbl || {}).length === 1;
          const isPlaceholder = isRepeatedPlacement || isRepeatedTitle || isLinkOnly;
          const existingRepVars = tbl?.repeated_prayer_variables || {};
          const repVarChoices = baseRepeatedVarOptions.filter((key) => {
            if (key === "season") return existingRepVars.season === undefined;
            if (key === "passToTable.dayOfTheWeek") return !(existingRepVars.passToTable || {}).dayOfTheWeek;
            return true;
          });
              ...
              <div className="hw-table-fields">
                ...
                {isRepeatedTitle && (
                  <>
                    ...
                    <div className="hw-repeated-vars">
                      ...
                      {tbl?.repeated_prayer_variables?.season !== undefined && (
                        <label className="hw-field-with-remove hw-field-inline">
                          <span>season</span>
                          <input
                            value={tbl?.repeated_prayer_variables?.season ?? ""}
                            onChange={(e) => setRepeatedVariable(idx, "season", e.target.value)}
                          />
                          <button type="button" className="hw-remove" onClick={() => removeRepeatedVariable(idx, "season")}>
                            🗑
                          </button>
                        </label>
                      )}
                      {tbl?.repeated_prayer_variables?.passToTable?.dayOfTheWeek !== undefined && (
                        <label className="hw-field-with-remove hw-field-inline">
                          <span>passToTable.dayOfTheWeek</span>
                          <input
                            value={tbl?.repeated_prayer_variables?.passToTable?.dayOfTheWeek ?? ""}
                            onChange={(e) => setRepeatedVariable(idx, "passToTable.dayOfTheWeek", e.target.value)}
                          />
                          <button type="button" className="hw-remove" onClick={() => removeRepeatedVariable(idx, "passToTable.dayOfTheWeek")}>
                            🗑
                          </button>
                        </label>
                      )}
                      {repVarChoices.length > 0 && (
                        <div className="hw-optional">
                          <select
                            value={repVarSelected}
                            onChange={(e) => setBaseOptionalSelect((prev) => ({ ...prev, [`rep-${idx}`]: e.target.value }))}
                          >
                            {repVarChoices.map((opt) => (
                              <option key={opt} value={opt}>
                                {opt}
                              </option>
                            ))}
                          </select>
                          <button type="button" className="secondary" onClick={() => setRepeatedVariable(idx, repVarSelected || repVarChoices[0], "")}>
                            Add variable
                          </button>
                        </div>
                      )}
                ...
                {isRepeatedPlacement && (
                  <>
                    ...
                    {tbl?.passToTable?.dayOfTheWeek !== undefined && (
                      <label className="hw-field-with-remove hw-field-inline">
                        <span>passToTable.dayOfTheWeek</span>
                        <input
                          value={tbl?.passToTable?.dayOfTheWeek ?? ""}
                          onChange={(e) => setPassToTable(idx, "dayOfTheWeek", e.target.value)}
                        />
                        <button type="button" className="hw-remove" onClick={() => removePassToTable(idx, "dayOfTheWeek")}>
                          🗑
                        </button>
                      </label>
                    )}
                    {placementChoices.length > 0 && (
                      <div className="hw-optional">
                        <select
                          value={placementSelected}
                          onChange={(e) => setBaseOptionalSelect((prev) => ({ ...prev, [`placement-${idx}`]: e.target.value }))}
                        >
                          {placementChoices.map((opt) => (
                            <option key={opt} value={opt}>
                              {opt}
                            </option>
                          ))}
                        </select>
                        <button type="button" className="secondary" onClick={() => setPassToTable(idx, "dayOfTheWeek", "")}>
                          Add variable
                        </button>
                      </div>
                    )}
